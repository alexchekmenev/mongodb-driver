...
describe('Query method', () => {
    ...
    test('get total count', async () => {
        const result = await mongoDbDriver.query(`SELECT count(*) as total
                                                  FROM \`test\`.donors`)
    ...
    test('implicit field, filter by city', async () => {
        const result = await mongoDbDriver.query(`SELECT count(*) \`donors__count\`
                                                  FROM donors AS \`donors\`
                                                  WHERE (\`Donor City\` = ?)
                                                  LIMIT 10000`, ["San Francisco"])
        expect(result).toEqual([{donors__count: 16925}])
    })

    test('implicit field, filter by is teacher', async () => {
        const result = await mongoDbDriver.query(`SELECT \`Donor State\` \`donors__donor_state\`,
                                                         count(*)        \`donors__count\`
                                                  FROM donors AS \`donors\`
    ...
    test('implicit field, filter by is teacher & state', async () => {
        const result = await mongoDbDriver.query(`SELECT \`Donor City\` \`donors__donor_city\`,
                                                         count(*)       \`donors__count\`
                                                  FROM donors AS \`donors\`
                                                  WHERE (donors.\`Donor Is Teacher\` = ?)
        ...
        expect(result).toEqual([
            ...
            { donors__donor_city: null, donors__count: 99 },
